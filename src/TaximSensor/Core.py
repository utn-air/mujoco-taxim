...
def rasterize_depth_from_trimesh(
    ...
    W: int,
    pixmm: float,
    vertices_h_cache: np.ndarray | None = None,
    faces_cache: np.ndarray | None = None,
) -> np.ndarray:
    """
    Returns zbuf (H,W) storing the largest negative z in the sensor frame for
    each pixel. Mesh vertices are converted from metres to millimetres before
    applying ``sTo``, so finite z-buffer values are in millimetres.
    """
